...
class StarsView: UIView {
  ...
  @objc private func tapped(with gestureRecognizer: UITapGestureRecognizer) {
    ...
    // Sound Reference: https://github.com/TUNER88/iOSSystemSoundsLibrary
  ...
  private func updateTypes(for currentRating: Double) {
    ...
    // There can only ever be 1 half star
    let halfStars = (5.0 - Double(fullStars) - Double(emptyStars) > 0.5) ? 1 : 0
    ...
    if let delegate = ratingDelegate {
      delegate.starsViewRatingDidChange(self, to: currentRating)
    }
  }
}
