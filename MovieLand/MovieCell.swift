...
class MovieCell: UICollectionViewCell {
  ...
  weak var touchDelegate: MovieCellDelegate?
  ...
  private func quickViewOpen(at point: CGPoint) {
    guard let delegate = touchDelegate else { return }
    delegate.movieCell(receivedTouchIn: self, at: point)
  }
  ...
  private func setupConstraints() {
    ...
    let contentTop = NSLayoutConstraint(
      item: containerView,
      attribute: .top,
      relatedBy: .equal,
      toItem: contentView,
    ...
    let imageY = NSLayoutConstraint(
      item: imageView,
      attribute: .centerY,
      relatedBy: .equal, toItem:
      containerView,
      attribute: .centerY,
      multiplier: 1.0,
      constant: 0.0)
